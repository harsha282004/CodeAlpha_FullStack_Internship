...
- **Phase 16** — security & authorization audit: an *actively tested*
  audit (every finding backed by a real request/socket connection against
  the real app, not a code read alone) covering authentication, password
  handling, OWNER/ADMIN/MEMBER authorization, cross-project IDOR
  (including explicit URL substitution), mass-assignment/field-injection
  resistance, input validation, Prisma/SQL safety (exactly one raw query
  in the codebase, a parameter-free `SELECT 1` health check), CORS/
  Helmet, Socket.IO security, and notification-ownership isolation. Zero
  new vulnerabilities found; the fix from Phase 12's room-join message
  distinction was re-verified as a regression check, not rediscovered.
  See [SECURITY_AUDIT.md](./SECURITY_AUDIT.md), including its explicit
  "Remaining limitations" section (no rate-limiting, no fuzzing, no
  deployed-instance pentest).
- **Phase 17** — automated testing: Vitest + Supertest +
  `socket.io-client` against the real Express app and an isolated
  `taskflow_test` PostgreSQL database (a second database in the same
  Docker container as `taskflow`, never the development database —
  `server/tests/setup.js` and `helpers.js`'s `resetDb()` each
  independently refuse to run unless `DATABASE_URL` points at it). 288
  tests across 14 files: one per resource area, plus a dedicated
  `security/` directory (isolation, an automated OWNER/ADMIN/MEMBER/
  non-member/unauthenticated matrix, mass-assignment, validation) and a
  `realtime/` directory driving real Socket.IO connections. Playwright's
  existing frontend suite (Phase 15) was extended, not replaced. See
  [TESTING.md](./TESTING.md) for the full per-file breakdown and how to
  run them.
- **Phase 18** — demo dataset: `prisma/seed-demo.js`, a small, curated,
  fictional dataset (6 users, 4 projects, 20 boards, 24 tasks, 48
  comments) kept deliberately separate from the pre-existing development
  seed (`prisma/seed.js`) via its own deterministic-id pattern, so both
  can coexist in one database without collision. Notifications are
  generated by calling the real `notification.service.js` functions the
  live app uses, not by inserting rows directly — verified idempotent by
  running it twice (47 notifications created the first run, 0 the
